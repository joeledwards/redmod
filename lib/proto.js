...
function parse (buffer, offset = 0) {
  switch (String.fromCharCode(buffer.readUInt8(offset))) {
    case '+': return readSimpleString(buffer, offset + 1)
    case '-': return readError(buffer, offset + 1)
    case ':': return readInteger(buffer, offset + 1)
    case '$': return readBulkString(buffer, offset + 1)
    case '*': return readArray(buffer, offset + 1)
